...
These five rules are enforced on **new and changed code** by a Codex
...
|-----------|--------------|--------------|
| `PostToolUse` hook ([`.Codex/settings.json`](.Codex/settings.json)) | After Codex edits a Go file | Runs the checker on that file and feeds any finding back so Codex self-corrects |
