...
func system_subservice_launch(servicePath string, startupMode bool) error {
	...
	if fileExists(servicePath + "/.noproxy") {
		...
		thisServicePort := subservice_getNextUsablePort()
...
func subservice_getSubserviceRoots() []string{
...
func subservice_getNextUsablePort() int {
	basePort := subserviceBasePort
	for subservice_checkIfPortInUse(basePort) {
		basePort++
	}
	return basePort
}

func subservice_checkIfPortInUse(port int) bool {
	for _, service := range runningSubServices {
