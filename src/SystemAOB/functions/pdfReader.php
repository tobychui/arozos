...
if (isset($_GET['filepath']) && $_GET['filepath'] != "" && isset($_GET['filename']) && $_GET['filename'] != ""){
	echo 'Loading sources from: ' . $_GET['filepath'] . '<br>';
	echo $_GET['filename'];
}else{
	die('ERROR. Undefined filepath or filename for opening.');
}


$filepath = $_GET['filepath'];
if (strpos($filepath,"/media") === 0){
    //This path start with /media, use extDiskAccess.php
    $filepath = "extDiskAccess.php?file=" . $filepath;
}else{
    $filepath = "../" . $filepath;
}
?>
...
if (inVDI){
		...
		parent.setWindowPreferdSize(windowID + "",1080,580);
}
$(document).ready(function(){
	if(window.mobilecheck()){
		window.open("<?php echo $filepath;?>");
	}else{
