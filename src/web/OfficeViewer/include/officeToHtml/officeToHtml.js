        ...
        req.onload = function (e) {
            ...
            */
            ...
            sheetNames.forEach(function (sheetName, idx) {
                ...
                /*
                    MAIN LOOP TO ITERATE ALL SHEETS FROM XLSX
                */
               let thisSheet = '';
               thisSheet += (`<table class="ui celled unstackable table">
                <thead>
                    <tr>
                        <th colspan="100%">${sheetName}</th>
                    </tr>
                </thead>
                <tbody>`);
                console.log(json);

                //Get the max width of table
                let maxWidthGrid = 0;
                let trimOffLine = json.length;
                let lineCounter = 0;
                let previousLineIsZero = false;
                json.forEach(function(line){
                    //Get the max width of the table
                    if (line.length > maxWidthGrid){
                        maxWidthGrid = line.length;
                    }

                    //Calculate early stop line
                    if (line.length == 0 && !previousLineIsZero){
                        trimOffLine = lineCounter;
                        previousLineIsZero = true;
                    }else if (previousLineIsZero && line.length > 0){
                        trimOffLine = lineCounter;
                        previousLineIsZero = false;
                    }
                ...
                lineCounter = 0;
                for (var x = 0; x < trimOffLine; x++){
                    let line = json[x];
                    thisSheet += (`<tr>`);
                    for (var i = 0; i < maxWidthGrid; i++){
                        let thisGridValue = line[i];
                        if (typeof(thisGridValue) != "undefined"){
                            thisSheet += (`<td>${thisGridValue}</td>`);
                        }else{
                            //Empty grid
                            thisSheet += (`<td></td>`);
                        }
                ...
                thisSheet += (`</tbody></table>`);
                $($container).append(thisSheet);
                /*
